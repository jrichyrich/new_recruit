...
export function createRosterPilotMcpServer(
  ...
  server.registerTool(
    "run",
    {
      ...
      description:
        "Research, build, modify, export, sync, compare exact rosters, or run Tessera stress tests locally or on the website. Build options use pointsLimit (not pointLimit). preferences accepts mobility, durability, objective, shooting, melee, elite, or horde; opponentAssumptions is { styleTags, source: 'user-stated' }. Other build fields include playerFaction, opponentFaction, legendsPolicy, collectionProfile, detachmentId, forceDispositionId, compareOpponentOptions, and comparisonDepth. Exact stress uses opponentRef; faction-portfolio stress uses options.opponentFaction, never both. Fresh exact local stress can select optional player abilities with selectedPlayerAbilityIds, inspect all optional activations with activationMode=envelope, and bind explicit leader/bodyguard selection IDs with selectedAttachmentBindings. Stress catalogueDriftMode defaults to reject and permits only reject or diagnostic. Pass references instead of full roster documents.",
      inputSchema: {
        action: z.enum([
          "research",
          "build",
